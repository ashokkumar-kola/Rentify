...
const defaultProfileImg = require('../../assets/images/profiles/default.png');
...
const ProfileScreen = ({ navigation }: { navigation: any }) => {
  ...
  const handleVerify = async (type: 'email' | 'phone') => {
    try {
      const endpoint =
        type === 'email' ? `/auth/forgot-password/${user._id}` : `/auth/verify-phone/${user._id}`;
  ...
  const handleValidateOtp = async () => {
    if (!otpValue) return Alert.alert('Missing OTP', 'Please enter the OTP');
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.profileHeader}>
        <Image source={defaultProfileImg} style={styles.avatar} />
      ...
      <View style={styles.menu}>
        ...
        <MenuItem icon="trash" label="Delete Account" color="red" onPress={handleDeleteAccount} />
        <MenuItem icon="sign-out" label="Logout" color="#e11d48" onPress={handleLogout} />
      </View>
            ...
            <View style={styles.modalOverlay}>
                <View style={styles.modalCard}>
                    <TouchableOpacity style={styles.closeButton} onPress={() => setOtpModalVisible(false)}>
                        <FontAwesome name="close" size={24} color="#333" style={{ alignSelf: 'flex-end' }} />
                    </TouchableOpacity>

                    <Text style={styles.modalTitle}>Gmail Verification</Text>
                    ...
                    <TouchableOpacity style={styles.verifyModelButton} onPress={handleValidateOtp}>
                        <Text style={styles.verifyButtonText}>Verify Account</Text>
                    </TouchableOpacity>

                    <Text style={styles.resendText}>
                        Didn't receive code?{' '}
                        <Text style={styles.resendLink} onPress={() => handleVerify(otpType || 'email')}>
                        Resend
                        </Text>
                    </Text>
                </View>
...
//     const handleValidateOtp = async () => {
//         if (!otpValue) {return Alert.alert('Enter OTP');}

//         try {
//             const endpoint =
//             otpType === 'email' ? '/auth/verify-email' : '/auth/verify-phone'; // '/verify-email/validate' : '/verify-phone/validate';
//             const res = await api.post(endpoint, {
//                 // userId: user._id,
//                 email: user.email,
//                 otp: otpValue,
